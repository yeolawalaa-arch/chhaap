...
/**
 * Google Gemini-backed language layer.
 *
 * Same split as the Anthropic provider: the Brand Brain owns every visual
 * decision, Gemini only sharpens names, taglines, voice and captions, and any
 * failure falls back to the deterministic heuristic engine rather than
 * breaking generation. This file is the second provider the Anthropic file's
 * own comment predicted — a file, not a refactor, because both implement the
 * same `AiProvider` interface over plain `fetch`.
 */
  ...
  private async complete(
    ...
    try {
      const res = await fetch(
        `${API_BASE}/${this.model}:generateContent?key=${this.apiKey}`,
        {
          method: "POST",
